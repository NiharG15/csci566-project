...
flags.DEFINE_string(
    'image_path', None,
    'Path to a folder of images. Overrides the config.')
flags.DEFINE_string(
    'tfds_name', None,
...
def run(config_map,
  ...
  if FLAGS.examples_path:
    config_update_map['%s_examples_path' % FLAGS.mode] = os.path.expanduser(
        FLAGS.examples_path)
  if FLAGS.image_path:
    config_update_map['%s_image_path' % FLAGS.mode] = os.path.expanduser(
        FLAGS.examples_path)
  if FLAGS.tfds_name:
    if FLAGS.examples_path:
      raise ValueError(
...
def main(unused_argv):
  ...
  print('okokokokok',configs.CONFIG_MAP)
