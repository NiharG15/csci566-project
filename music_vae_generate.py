...
flags.DEFINE_string(
  'input_image', None,
  'Path to the image file used to generate conditioned z.'
)
...
def run(config_map):
  ...
  if FLAGS.mode == 'interpolate':
    ...
    with model._sess as sess:
      ...
      mu = mu.eval()
      sigma = sigma.eval()
      latent = ds.MultivariateNormalDiag(loc=mu + mu_music, scale_diag=sigma + sigma_music).sample().eval()
      results = model.decode(length=config.hparams.max_seq_len, z=latent, temperature=FLAGS.temperature)
      print(results)
